...
import static CardNotPresent.Home.ui.Home.HomeFragment.PERSON_CONDITION_METHOD;
import static CardNotPresent.Main.MainActivity.USER_ID;
import static CardNotPresent.before_main.Splash_Activity.WEB_API;

public class Persons_Transaction_ConditionViewModel extends ViewModel {

    private MutableLiveData<List<PersonsCondition_Model>> perConditionModelList;
    private String receivedToken = "";
//    AVLoadingIndicatorView progressBar;

    LinearLayout linearLayout;

    public Persons_Transaction_ConditionViewModel() {

    }

    public void findLiveDatas() {
        linearLayout.setVisibility(View.VISIBLE);
        perConditionModelList = new MutableLiveData<>();
    ...
    private void retrievePersonsDataWithAccessToken(String receivedToken) {
        ...
        Model_ResultStatus_Per_Conditions model_resultStatus_per_trans = new Model_ResultStatus_Per_Conditions(USER_ID);

        Call<Model_ResultStatus_Per_Conditions> callPost = jsonPlaceHolderApi.getPersonalConditionListData(model_resultStatus_per_trans);

        callPost.enqueue(new Callback<Model_ResultStatus_Per_Conditions>() {
            @Override
            public void onResponse(@NotNull Call<Model_ResultStatus_Per_Conditions> call, @NotNull Response<Model_ResultStatus_Per_Conditions> response) {
                if (!response.isSuccessful()) {
                    Log.d("Response Not Successful", "Response Code: " + response.code());
                    return;
                }
                ...
                Model_ResultStatus_Per_Conditions userData = response.body();

                assert userData != null;
                if (userData.getUsernameValidationPersonCondition().equals("true"))
                {
                    List<PersonsCondition_Model> pModelList = new ArrayList<>();

                    for (int i = 0; i < userData.getPerConditionModelList().size(); i++) {
                        PersonsCondition_Model perModel = userData.getPerConditionModelList().get(i);
                        ...
                        String strTransactionStatus = perModel.getTransactionStatus().trim().toLowerCase();
                        ...
                        if (strTransactionStatus.equals("true") && PERSON_CONDITION_METHOD == 2) {
                            pModelList.add(perModel);
                        }
                        else {
                            if (strTransactionStatus.equals("false") && PERSON_CONDITION_METHOD == 1) {
                                pModelList.add(perModel);
                            }
                        }
                    }

                    perConditionModelList.setValue(pModelList);
                }
                else
                {
            ...
            @Override
            public void onFailure(@NotNull Call<Model_ResultStatus_Per_Conditions> call, Throwable t) {
                Log.d("Response Failure", "Failure Code: " + t.getMessage());
            }
        });
    }

    public MutableLiveData<List<PersonsCondition_Model>> getPerConditionModelList() {
